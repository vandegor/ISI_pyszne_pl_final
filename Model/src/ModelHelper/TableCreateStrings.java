...
public enum TableCreateStrings {
	...
	/***/
	TRANZAKCJA("		CREATE TABLE `tranzakcja` 			(`id` INTEGER PRIMARY KEY AUTOINCREMENT , `kontoPlus_id` INTEGER NOT NULL , `kontoMinus_id` INTEGER , `kwota` FLOAT , `data` TIMESTAMP, 'tytul' VARCHAR, FOREIGN KEY(kontoMinus_id) REFERENCES konto(id),  FOREIGN KEY(kontoPlus_id) REFERENCES konto(id) )"),
	/***/
